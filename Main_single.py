...
data_collection = ["1_1_06"]

for i in data_collection:
    path = 'D:\CuiProject\PythonProject\Single-file\_trajectories\_' + i + '.txt'
    agent, neighbor = data_process.get_training(np.array(data_process.get_data(path)))
    neighbor = neighbor.reshape(N, 127, 36)
...
if __name__ == "__main__":
    ...
    for epoch in range(n_epochs):
        ...
        for mini_batch in train_dl:
            ...
            loss = criterion(out, agent_target.reshape(batch_size, 252))
            loss.backward()
            optimizer.step()
        ...
        for mini_batch in valid_dl:
            ...
            valid_loss = criterion(valid_out, agent_target.reshape(batch_size, 252))
